...
from .models import Team, TeamMembership, Project, Species, TaskBatch, UserProfile
...
def team_detail_view(request, team_id):
    ...
    # Debug output
    print(f"TeamMembership check: User {request.user.username}, Team {team.name} (ID: {team.id}), Membership exists: {membership_exists}")
    if not membership_exists:
        print(f"  - User's active team: {request.user.profile.team.name if request.user.profile.team else 'None'}")
        print(f"  - User's memberships: {list(TeamMembership.objects.filter(user=request.user).values_list('team__name', flat=True))}")
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                membership, created = TeamMembership.objects.get_or_create(
                    user=request.user,
                    team=team,
                    defaults={'is_admin': True}  # Team creator is always admin
                )
...
def debug_teams_view(request):
    ...
    profiles_without_membership = UserProfile.objects.filter(
        team__isnull=False
    ).exclude(
        user__in=TeamMembership.objects.filter(
            team=models.F('user__profile__team')
        ).values_list('user', flat=True)
    )
    
    # Find users with memberships but no active team
    users_with_memberships_no_team = UserProfile.objects.filter(
        user__in=TeamMembership.objects.values_list('user', flat=True),
        team__isnull=True
    )
