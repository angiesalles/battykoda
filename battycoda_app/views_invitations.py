...
from .models import Team, TeamInvitation, UserProfile, TeamMembership, User
...
def team_users_view(request):
    ...
    active_invitations = TeamInvitation.objects.filter(
        team=team,
        accepted=False
...
def accept_invitation_view(request, token):
    ...
    if request.user.is_authenticated:
        ...
        messages.info(
            request, 
            "Please register or log in to accept your team invitation."
        )
